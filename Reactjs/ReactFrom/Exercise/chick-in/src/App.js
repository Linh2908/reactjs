...
function App () {
  ...
  const handleChange = (e) => {
    const value = e.target.type === "checkbox" ? !form[e.target.name] : e.target.value;
    setForm({
      ...form,
      [e.target.name] : value
    })
  ...
  const handleValidate = () => {
    ...
    }else if(form.yearOfBirth > (new Date().getFullYear())) {
        ...
        {({errors, handleSubmit})=> (
          ...
          <div className={`custom-input ${errors.name ? 'custom-input-error' : ''}`} >
            ...
            <div className={`custom-input ${errors.yearOfBirth ? 'custom-input-error' : ''}`} >
              ...
              <input name="yearOfBirth" value={form.yearOfBirth || ''} onChange={handleChange}/>
